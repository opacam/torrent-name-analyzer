...
import re
from torrent_name_analyzer.name_parser.patterns import (
    delimiters,
    episode_pattern,
    patterns,
    types,
)
from torrent_name_analyzer.name_parser.exceptions import parse_exceptions
...
class PTN(object):
    ...
    def parse(self, name):
        ...
        for key, pattern in patterns:
            ...
            # With multiple matches, we will usually want to use the first
            # match. For 'year', we instead use the last instance of a year
            # match since if a title includes a year, we don't want to use this
            # for the year field.
            match_index = 0
            if key == "year":
                match_index = -1
            ...
            # patterns for multi season/episode make the range,
            # and only the range, appear in match[0]
        ...
        match = re.findall(
            r"((?:(?:[A-Za-z][a-z]+|[A-Za-z])(?:[. \-+_]|$))+)", clean
        )
        if match:
            match = re.findall(
                episode_pattern + r"[._\-\s+]*(" + re.escape(match[0]) + ")",
                self.torrent["name"],
                re.IGNORECASE,
            )
            if match:
                # get longest string...which probably is the episode title
                episode_string = max(match[0], key=len)
                self._part(
                    "episodeName",
                    match[0],
                    episode_string,
                    self._clean_string(episode_string),
                )
                clean = clean.replace(episode_string, "")
        ...
        if len(clean) != 0 and not self.group_raw:
        ...
        if len(clean) != 0:
            if len(clean) == 1:
                clean = clean[0]  # Avoids making a list if it only has 1 element  # noqa: 501
